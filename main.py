...
# 设置streamlit页面配置
st.set_page_config(layout="wide", page_title="Prediction model for in-hospital all-cause mortality in patients with acute pancreatitis complicated by sepsis.")

# 在侧边栏显示模型推荐和特征解释的警告
st.sidebar.warning('The remaining dimension variables are values within the first 24 hours after admission to the ICU')

# 显示预后模型的信息
st.info('#### Prognostic model for acute pancreatitis complicated by sepsis')

# 指定要使用的模型文件
model_file = "AQlearner.pkl"  # 在这里使用你的模型文件
